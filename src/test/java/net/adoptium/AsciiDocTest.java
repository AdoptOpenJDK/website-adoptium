...
class AsciiDocTest {
    ...
    //Evtl. Setup methode aber momentan auf Eis
    @Test
    void AsciiDocCheckIfHtmlPagesExist() {
        try (Playwright playwright = Playwright.create()) {
            ...
            for (BrowserType browserType : browserTypes) {
                ...
                Object[] filesObjectArray = Arrays.stream(Objects.requireNonNull(dir.listFiles()))
                        .filter(File::isFile).toArray();
                ...
                for (File file : filesInDir) {
                    try (Browser browser = browserType.launch()) {
                        ...
                        try {
                            ...
                            fail("Failed because http://127.0.0.1:8081/docs/" + file.getName()
                                    + " does not exist when opened with browser", e);
                        }
                    } catch (PlaywrightException e) {
