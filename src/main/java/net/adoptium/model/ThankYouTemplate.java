...
public class ThankYouTemplate {
    ...
    public ThankYouTemplate(final Map<DownloadArgumentGroup, String> versionDetails, final Binary binary) {
        ...
        if (binary.getInstaller() != null) {
            final Installer installer = binary.getInstaller();
            downloadLink = installer.getLink();
            checksum = installer.getChecksum();
        } else {
            final Package pkg = binary.getPackage();
